...
function AssignCell({ cell, doCellChange, task, taskChange }) {
  ...
  const iconsSpacing = () => {
    const widthX = wrapperRef.current
      ? Math.floor(wrapperRef.current.offsetWidth / 80) ** 2
      : 2;
    if (task.assignedTo.length >= 0 && task.assignedTo.length < 8)
      return -8 - task.assignedTo.length + widthX;
    return -16 + widthX;
  };
    ...
    >
      <div className={style['assigned-users-wrapper']}>
        {task.assignedTo.map((user) => (
          <div
            key={user._id}
            className={style['user-icon-wrapper']}
            style={{ marginRight: `${iconsSpacing()}px` }}
          >
            <UserIcon
              key={user._id}
              userName={user.name}
      ...
      <button
        className={style['add-assigned-btn']}
